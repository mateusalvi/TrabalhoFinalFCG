...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 800, "INF01047 - 229787 - Mateus Luiz Salvi", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    while (!glfwWindowShouldClose(window))
    {
                ...
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    ...
                    PushMatrix(model); // Guardamos matriz model atual na pilha
                        ...
                        PushMatrix(model);
                            ...
                            PushMatrix(model); // Guardamos matriz model atual na pilha
                                model = model * Matrix_Scale(0.9f, 0.1f, 0.9f); // Atualizamos matriz model (multiplicação à direita) com um escalamento do antebraço direito
                                glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                                DrawCube(render_as_black_uniform); // #### ANTEBRAÇO DIREITO // Desenhamos o antebraço direito
                            PopMatrix(model);
                    PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
        ...
        //Braço esquerdo
        PushMatrix(model);
            model = model * Matrix_Translate(1.1f, 0.0f, 0.0f);
            PushMatrix(model);
                model = model
                ...
                PushMatrix(model);
                    ...
                    PushMatrix(model);
                        ...
                        PushMatrix(model);
                            model = model * Matrix_Translate(0.0f, -1.1f, 0.0f);
                            PushMatrix(model);
                                model = model * Matrix_Scale(0.9f, 0.1f, 0.9f);
                                glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
                                DrawCube(render_as_black_uniform);
                            PopMatrix(model);
                    PopMatrix(model);
        ...
        //Perna direita
        PushMatrix(model);
            model = model * Matrix_Translate(-0.75f, -1.05f, 0.0f);
            PushMatrix(model);
                model = model * Matrix_Scale(0.35f, 0.6f, 0.2f);
                glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
                DrawCube(render_as_black_uniform);
            PopMatrix(model);
            PushMatrix(model);
                model = model * Matrix_Translate(0.0f, -0.65f, -0.0f);
                PushMatrix(model);
                    model = model * Matrix_Scale(0.3f, 0.55f, 0.2f);
                    glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
                    DrawCube(render_as_black_uniform);
                PopMatrix(model);
                PushMatrix(model);
                    model = model * Matrix_Translate(0.0f, -0.6f, 0.1f);
                    PushMatrix(model);
                        model = model * Matrix_Scale(0.25f, 0.1f, 0.4f);
                        glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
                        DrawCube(render_as_black_uniform);
                    PopMatrix(model);
                PopMatrix(model);
